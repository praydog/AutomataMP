...
#include <imgui.h>
#include "automata-imgui/font_robotomedium.hpp"
#include "automata-imgui/imgui_impl_dx11.h"
#include "automata-imgui/imgui_impl_dx12.h"
#include "automata-imgui/imgui_impl_win32.h"
...
std::unique_ptr<AutomataMP> g_framework{};

void AutomataMP::hook_monitor() {
    ...
    if (d3d11 == nullptr || d3d12 == nullptr 
        || (renderer_type == AutomataMP::RendererType::D3D11 && d3d11 != nullptr && !d3d11->is_inside_present()) 
        || (renderer_type == AutomataMP::RendererType::D3D12 && d3d12 != nullptr && !d3d12->is_inside_present())) 
    {
    ...
    if (ntdll != nullptr) {
        ...
        if (rtl_get_version != nullptr) {
            ...
            if (rtl_get_version(&os_version_info) != 0) {
                ...
                spdlog::info("Disclaimer: AutomataMP does not send this information to the developers or any other third party.");
                spdlog::info("This information is only used to help with the development of AutomataMP.");
            }
        } else {
...
AutomataMP::~AutomataMP() {
    spdlog::info("AutomataMP shutting down...");
...
void AutomataMP::run_imgui_frame(bool from_present) {
...
// D3D11 Draw funciton
void AutomataMP::on_frame_d3d11() {
    std::scoped_lock _{ m_imgui_mtx };
    ...
    if (!m_initialized) {
        if (!initialize()) {
            spdlog::error("Failed to initialize AutomataMP on DirectX 11");
            return;
        }

        spdlog::info("AutomataMP initialized");
        m_initialized = true;
        return;
    }
...
void AutomataMP::on_post_present_d3d11() {
...
// D3D12 Draw funciton
void AutomataMP::on_frame_d3d12() {
    std::scoped_lock _{ m_imgui_mtx };
    ...
    if (!m_initialized) {
        if (!initialize()) {
            spdlog::error("Failed to initialize AutomataMP on DirectX 12");
            return;
        }
...
void AutomataMP::on_post_present_d3d12() {
    if (!m_error.empty() || !m_initialized || !m_game_data_initialized) {
        if (m_last_present_time <= std::chrono::steady_clock::now()){
            m_last_present_time = std::chrono::steady_clock::now();
        }
...
bool AutomataMP::on_message(HWND wnd, UINT message, WPARAM w_param, LPARAM l_param) {
    m_last_message_time = std::chrono::steady_clock::now();
...
// this is unfortunate.
void AutomataMP::on_direct_input_keys(const std::array<uint8_t, 256>& keys) {
    /*const auto menu_key = AutomataMPConfig::get()->get_menu_key()->value();
...
void AutomataMP::set_draw_ui(bool state, bool should_save) {
    std::scoped_lock _{m_config_mtx};
    ...
    if (m_game_data_initialized) {
        AutomataMPConfig::get()->get_menu_open()->value() = state;
    }
...
int AutomataMP::add_font(const std::filesystem::path& filepath, int size, const std::vector<ImWchar>& ranges) {
    ...
    m_additional_fonts.emplace_back(AutomataMP::AdditionalFont{filepath, size, ranges, (ImFont*)nullptr});
    m_fonts_need_updating = true;

    return m_additional_fonts.size() - 1;
}

void AutomataMP::update_fonts() {
...
void AutomataMP::invalidate_device_objects() {
...
void AutomataMP::draw_ui() {
    ...
    ImGui::Begin("AutomataMP", &m_draw_ui);
    ...
    if (m_error.empty() && m_game_data_initialized) {
        ...
        ImGui::TextWrapped("AutomataMP is currently initializing...");
        ImGui::TextWrapped("This menu will close after initialization if you have the remember option enabled.");
    } else if (!m_error.empty()) {
        ImGui::TextWrapped("AutomataMP error: %s", m_error.c_str());
    }
...
void AutomataMP::draw_about() {
    ...
    ImGui::Text("https://github.com/praydog/AutomataMP");
...
void AutomataMP::set_imgui_style() noexcept {
...
bool AutomataMP::initialize() {
    ...
    if (m_first_frame) {
        ...
        std::thread init_thread([this]() {
            try {
                //automatamp::initialize_sdk(); // TODO
...
bool AutomataMP::initialize_windows_message_hook() {
    if (m_wnd == 0) {
        return false;
    }
